...
MentorBot.prototype._onMessage = async function (message) {
	if(this._isChatMessage(message) &&
		...
		if (missedShift) {
			...
			this._getFreeMentors(sender.profile.display_name, message);
			return;
		}
...
MentorBot.prototype._getMentorID = async function (mentorName) {
	var self = this;

	var query = 'SELECT id FROM mentors WHERE name = "'+mentorName+'";';
	var mentorID = null;
	await self.db.get(query, async function (err, record) {
		if (err) {
			console.log("ERROR");
			return false;
		}
...
MentorBot.prototype._getFreeMentors = async function (mentorName, message) {
	var self = this;
	...
	await self.db.get(query, async function (err, record) {
		...
		if (mentorID == null) {
			self._sendMessage('Hey, '+mentorName+'! You currently do not have a shift allocated, so don\'t worry!'
				, self._getChannelById(message.channel).name);
			return false;
		}

		self._sendMessage('Hey, '+mentorName+'! Can you please fill out the Missed Shifts form? \
				'+links['missedShiftForm'], self._getChannelById(message.channel).name);

		var queryIn = 'SELECT freestudents, time FROM shifts WHERE studentid = '+mentorID+';';
		await self.db.get(queryIn, async function (err, record) {
			if (err) {
				//return console.error('DATABASE ERROR:', err);
				console.log("ERROR");
				return false;
			...
			self._sendMessage('Listen Up: '+record['FreeStudents']+'! Are any of you available to cover '+mentorName+'\'s shift at '+record['Time']+'?'
				, self._getChannelById(message.channel).name);
			return record;
